...
export default function CallLoop() {
  return (
    <section className="py-16 sm:py-[120px] px-4 sm:px-6 relative overflow-hidden">
      ...
      <div className="absolute -top-32 left-1/2 h-[320px] w-[640px] -translate-x-1/2 rounded-full bg-primary/10 blur-[140px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto w-full relative z-10">
        ...
        >
          ...
          <h2 className="text-4xl sm:text-5xl md:text-7xl font-black font-display leading-[1.05] tracking-tight mb-4">
            CALL. REVIEW.<br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary via-[#D8FF86] to-secondary">ADAPT.</span>
          </h2>
          ...
          {moments.map((moment, idx) => {
            ...
            return (
              <motion.div
                ...
                className="glass-card rounded-[24px] p-6 sm:p-8 border border-white/10 hover:border-primary/40 transition-all duration-300 hover:shadow-[0_20px_50px_rgba(141,255,106,0.15)]"
              >
                <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-gradient-to-br from-primary to-secondary flex items-center justify-center mb-5 shadow-lg shadow-primary/20">
                  <Icon className="text-[#070A0D]" size={24} />
                </div>
                <h3 className="text-xl sm:text-2xl font-bold font-display tracking-tight mb-3 text-textPrimary">{moment.title}</h3>
                <p className="text-sm sm:text-[15px] text-textSecondary leading-relaxed font-normal">{moment.text}</p>
              </motion.div>
            );
          })}
        </div>
        ...
        <motion.div
          ...
          className="mt-6 sm:mt-8 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 rounded-[24px] border border-white/10 bg-[#111A22]/90 px-6 py-5 shadow-glass"
        >
          <div>
            <p className="text-sm sm:text-base text-textSecondary">
              <strong className="text-textPrimary font-semibold">Use the loop when motivation drops.</strong> It is the simplest way to make accountability feel less like a tracker and more like a coach.
            </p>
          </div>
          <div className="flex flex-wrap gap-3">
            <a href="/fitness-app-that-calls-you/" className="inline-flex items-center gap-2 rounded-full border border-primary/40 bg-primary/10 px-4 py-2 text-sm font-semibold text-primary hover:bg-primary/20 transition-colors">
              Read the guide
              <ArrowRight size={16} />
            </a>
