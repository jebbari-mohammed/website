...
export default function HowItWorks() {
  return (
    <section id="how" className="py-16 sm:py-[120px] px-4 sm:px-6 bg-gradient-to-b from-transparent via-[#0E151B]/60 to-transparent relative">
      <div className="max-w-7xl mx-auto w-full">
        ...
        >
          ...
          <h2 className="text-3xl sm:text-4xl md:text-6xl font-black font-display leading-[1.05] tracking-tight mb-4">
            FROM DOWNLOAD TO<br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary via-[#D8FF86] to-secondary">TRANSFORMATION.</span>
          </h2>
          <p className="text-sm sm:text-[17px] text-textSecondary max-w-[560px] leading-relaxed font-sans font-normal">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mt-10 sm:mt-14 relative">
          ...
          <div className="hidden lg:block absolute top-[60px] left-[12.5%] right-[12.5%] h-[1px] bg-gradient-to-r from-primary/40 via-secondary/40 to-tertiary/40" />
          ...
          {steps.map((step, idx) => {
            ...
            return (
              <motion.div
                ...
                className="glass-card rounded-[20px] sm:rounded-[24px] p-6 sm:p-8 relative group hover:border-primary/35 hover:-translate-y-2 transition-all duration-300"
              >
                {/* Step number + icon */}
                <div className="relative mb-4 sm:mb-6">
                  ...
                  <span className="absolute -top-2 -left-2 text-[40px] sm:text-[48px] font-black font-display text-white/10 leading-none z-0">
                    {step.num}
                  </span>
                </div>

                <h3 className="text-lg sm:text-xl font-bold mb-2 sm:mb-3 font-display text-textPrimary">{step.title}</h3>
                <p className="text-xs sm:text-sm text-textSecondary leading-[1.7] font-sans mb-3 sm:mb-4">
                  {step.desc}
                </p>

                {/* Highlight tag */}
                <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-primary/10 border border-primary/25 text-[10px] sm:text-[11px] text-primary font-bold uppercase tracking-wider">
                  {step.highlight}
                </div>
              </motion.div>
            );
